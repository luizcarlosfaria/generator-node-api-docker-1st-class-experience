...
export class HealthCheckApiController implements IApiController {
	public register(server: restify.Server): void {
		...
		server.get("/", async (req, res, next) => {
			...
			await collection.insert({ Message: "generator-node-api-docker-1st-class-experience is a 1st class awesome experience with typescript, node, docker and vscode!"});
